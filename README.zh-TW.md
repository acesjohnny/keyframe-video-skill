...
<p align="center">
  <a href="examples/kunlun-one-leaf/kunlun-one-leaf.mp4"><img src="examples/kunlun-one-leaf/preview.gif" width="480" alt="三鏡頭範例：兩隻靈蟲乘葉飛過瀑布、躲過巨鳥、抵達瑤池"></a><br>
  <sub>用這套流程製作的 15 秒三鏡頭範例，點擊觀看 MP4。</sub>
</p>
...
| **4 · 託管** | 已核准的關鍵影格 | 暫時性的公開網址 | 單獨核准；上傳資料夾先掃描；每個網址重新下載比對雜湊值；正式環境不動 |
...
| 鏡頭 | 時長 | 畫面與運動 | 聲音 |
|---|---|---|---|
| 01 · 一葉凌空 | 5 秒 | 兩隻靈蟲乘一片綠葉從瀑布旁升起，鏡頭跟隨並輕輕後拉，穿過水霧與晨光 | 瀑布聲、微風；無對白 |
| 02 · 雲隙驚影 | 5 秒 | 一隻巨鳥逼近，葉子向左側轉、鑽過雲縫，兩隻靈蟲伏低 | 風聲漸強、遠處鳥鳴；無對白 |
| 03 · 瑤池初見 | 5 秒 | 葉子停在桃枝旁，五色瑤池與桃林展開，一隻紅鳥飛遠 | 風停、鳥鳴；無對白 |

它經過各個階段的方式：選定一場戲、核准三鏡頭分鏡（第 1 步）；寫好提示詞並核准（第 2 步）；產生三張關鍵影格並檢查（第 3 步）；託管到一個 24 小時後自動失效的暫時預覽頻道（第 4 步）；每個鏡頭送出一個 `agnes-video-2.5-flash` 任務，首格模式、720p（第 5 步）；片段正規化後串接（第 6 步）。

這是這套流程**第一次**完整跑通。`SKILL.md` 裡現在的大部分內容——走位、角色卡、提示詞語言規則、失敗分類——都是後來更大的專案在新的地方出錯之後才補上的。放在這裡是為了呈現整條流水線從頭到尾的樣子，而不是展示每一條規則。
...
把 skill 複製到你的 Agent 的 skills 資料夾，例如：
